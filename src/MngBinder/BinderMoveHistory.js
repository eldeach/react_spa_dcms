...
function BinderMoveHistory(){
    ...
    function authCheck(){
        if(cookies.load('loginStat')){
            if(cookies.load('userInfo').user_auth.indexOf("VIEW_BINDER_MOVE_HISTORY",0)!=-1){
                setTblCtrl(true)
            }
            else{
                alert("VIEW_BINDER_MOVE_HISTORY 권한이 없습니다.")
                navigate('/')
            }
    ...
    return(
        <div style={{padding:'0.5vw'}}>
            <div>
                <MngTable getUrlStr={'/getbindermovehistory'} targetPk={{}} heightValue={'75vh'} tblCtrl={tblCtrl} chkSel={false} deleteButton={false} addToListButton={false} editable={false} selectButton={false}/>
            </div>
            ...
            <Paper sx={{ position: 'fixed', bottom: 0, left: 0, right: 0, padding:'10px' }} elevation={6}>
                <div style={{width:'100%', display:'flex', alignItems:'center', backdropFilter:'blur(10px)'}}>
                    ...
                    <Typography variant="BUTTON" component="div" sx={{ flexGrow: 1, overflow:'hidden', marginLeft:'4px' }}>{"바인더 입출고 이력"}</Typography>
